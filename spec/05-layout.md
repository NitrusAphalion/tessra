...
The colocated checkout directory is a workspace owned by the daemon's human principal, the repository owner. An agent session started in it adopts it with `workspace --action adopt`, which records the agent (the session's parent, so every session of that agent finds it again) as the workspace's holder: the agent's edits and snapshots are made there, and the owner's own `edit`, `snapshot`, and `rewind` of the checkout are refused with `HELD` until the holder or the owner runs `workspace --action release`. Landing what the holder snapshotted there, and exporting it, need no release. Adoption is refused while the checkout carries another principal's unlanded revision, which would otherwise become the adopter's next revision. The daemon watches `.git/refs` and imports new commits as they appear, and snapshots the checkout on each git commit and on demand, so a team still using git on the same directory never diverges.

`export --format git --branch <b>` writes every landed trunk revision that is not yet a commit as one commit on top of the newest that is, in trunk order, authored by the change's author with `Tessra-Change` and `Tessra-Revision` trailers, updates the branch ref, and pushes on request. A checkout of the branch is fast-forwarded when it is clean; when it already holds the exported content, as it does when the landed change was made in it, HEAD and the index move to the tip and no file is touched; otherwise it is left alone and the result says so. With `git.export=<branch>` in the machine-local configuration, every landing runs the export, and with `git.push=<remote>` the push, and the landing's result carries the outcome under `export`. The owner's `status` reports `git`: the branch, the landed revisions not yet exported, and the commits not yet imported. `import --branch <b>` lands every commit the branch gained since, first-parent order, through the normal landing. `import --branch <b> --history` lands them the way `init --history` does instead: each commit's tree becomes the next trunk revision with a legacy intent, outside the standard, with no verifiers and no hooks, and it takes the owner credential. That is for history that already exists and was checked elsewhere; a commit that is really a proposal goes through the standard. After either moves the checkout, the colocated workspace follows: it is pointed at the landed revision HEAD's commit maps to, unless its current revision is not landed, which is unlanded work of the owner's that landing restacks; the result reports `workspace` as followed, at HEAD, or left alone with why. The revision-to-commit mapping is store metadata under `git:rev:<hex>` and `git:commit:<hash>`, plus the commit hash imported revisions name in their body, so a round trip rewrites nothing. Export is linear; landing merges are not exported as git merges.

The tracking rules' ignored class is what git would ignore: the global excludes file, the checkout's `.git/info/exclude`, the root `.gitignore`, and every nested `.gitignore` scoped to its directory, later files winning as in git. A snapshot reads them afresh, so an ignore file added since the last snapshot takes effect at once, and a file git never sees is never a file of the repository. `export --format git` asks git itself, through `check-ignore`, and leaves out of a commit whatever git would ignore, so a snapshot taken before the rules knew an ignore file still exports clean; the result counts them under `ignored`. `.gitattributes` `eol` seeds `eol`.
