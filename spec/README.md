...
- Git import is first-parent and lands each commit as its own change through the coordinator; a commit that conflicts with trunk stops the import there. Content round-trips byte for byte inside Tessra; a checkout's line endings follow git's own settings. Executable bits and symlinks round-trip; submodule pointers, signed commits, notes, and tags do not.
...
- Structural similarity in node matching is not implemented; a unit that changed both name and body gets a fresh ID unless a recorded `rename` names it. Of the `revision.ops` kinds only `rename` is recorded.
