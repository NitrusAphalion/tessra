...
A git commit landed as history (`import --history`) takes none of these steps: its tree is the next revision on the head, its intent is legacy, the op is the owner's `import` citing no attestation, and no hook fires. It is the same landing `init` gives the commits it imports, and 04-security.md step 7 admits it from an owner and from nobody else.
